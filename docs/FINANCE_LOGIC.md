...
3. **Actionable always** — every insight ends with something the user *can do today*
...
function calcHealthScore(data: FinancialContext): number {
  ...
  const cashflowScore = clamp(data.savingsRate / 0.20, 0, 1) * 25;
  ...
  const savingsScore = clamp(data.savingsRate / 0.20, 0, 1) * 20;
  ...
  const highRateProportion = data.highInterestDebtBalance / Math.max(data.totalDebt, 1);
  const hirScore = clamp(1 - highRateProportion, 0, 1) * 15;

  return Math.round(cashflowScore + dtiScore + savingsScore + efScore + hirScore);
}
```
...
- Always show the *interest cost difference* between strategies in £
